...
pub fn getTickAtSqrtRatio(sqrt_price_x96: U160) -> Int24 {
    ...
    assert!(
        sqrt_price_x96.ge(&U160::from_dec_str(MIN_SQRT_RATIO).unwrap())
            && sqrt_price_x96.lt(&U160::from_dec_str(MAX_SQRT_RATIO).unwrap()),
        "R"
    );
    ...
    let log_sqrt10001 =
        log_2.saturating_mul(U256::from_dec_str("255738958999603826347141").unwrap());
    ...
    if log_2_is_position {
        if log_sqrt10001.ge(&log_sub_const) {
            ...
            tickLow = Int24::try_from(log_sqrt10001_temp.as_u32()).unwrap();
        } else {
        ...
        tickLow = -Int24::try_from(log_sqrt10001_temp).unwrap();
    }
...
pub fn getSqrtRatioAtTick(tick: Int24) -> U160 {
    ...
    if !(abs_tick & U256::from(0x2)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xfff97272373d413259a46990580e213a").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x4 != 0) ratio = (ratio * 0xfff2e50f5f656932ef12357cf3c7fdcc) >> 128;
    if !(abs_tick & U256::from(0x4)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xfff2e50f5f656932ef12357cf3c7fdcc").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x8 != 0) ratio = (ratio * 0xffe5caca7e10e4e61c3624eaa0941cd0) >> 128;
    if !(abs_tick & U256::from(0x8)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xffe5caca7e10e4e61c3624eaa0941cd0").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x10 != 0) ratio = (ratio * 0xffcb9843d60f6159c9db58835c926644) >> 128;
    if !(abs_tick & U256::from(0x10)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xffcb9843d60f6159c9db58835c926644").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x20 != 0) ratio = (ratio * 0xff973b41fa98c081472e6896dfb254c0) >> 128;
    if !(abs_tick & U256::from(0x20)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xff973b41fa98c081472e6896dfb254c0").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x40 != 0) ratio = (ratio * 0xff2ea16466c96a3843ec78b326b52861) >> 128;
    if !(abs_tick & U256::from(0x40)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xff2ea16466c96a3843ec78b326b52861").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x80 != 0) ratio = (ratio * 0xfe5dee046a99a2a811c461f1969c3053) >> 128;
    if !(abs_tick & U256::from(0x80)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xfe5dee046a99a2a811c461f1969c3053").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x100 != 0) ratio = (ratio * 0xfcbe86c7900a88aedcffc83b479aa3a4) >> 128;
    if !(abs_tick & U256::from(0x100)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xfcbe86c7900a88aedcffc83b479aa3a4").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x200 != 0) ratio = (ratio * 0xf987a7253ac413176f2b074cf7815e54) >> 128;
    if !(abs_tick & U256::from(0x200)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xf987a7253ac413176f2b074cf7815e54").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x400 != 0) ratio = (ratio * 0xf3392b0822b70005940c7a398e4b70f3) >> 128;
    if !(abs_tick & U256::from(0x400)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xf3392b0822b70005940c7a398e4b70f3").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x800 != 0) ratio = (ratio * 0xe7159475a2c29b7443b29c7fa6e889d9) >> 128;
    if !(abs_tick & U256::from(0x800)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xe7159475a2c29b7443b29c7fa6e889d9").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x1000 != 0) ratio = (ratio * 0xd097f3bdfd2022b8845ad8f792aa5825) >> 128;
    if !(abs_tick & U256::from(0x1000)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xd097f3bdfd2022b8845ad8f792aa5825").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x2000 != 0) ratio = (ratio * 0xa9f746462d870fdf8a65dc1f90e061e5) >> 128;
    if !(abs_tick & U256::from(0x2000)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0xa9f746462d870fdf8a65dc1f90e061e5").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x4000 != 0) ratio = (ratio * 0x70d869a156d2a1b890bb3df62baf32f7) >> 128;
    if !(abs_tick & U256::from(0x4000)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0x70d869a156d2a1b890bb3df62baf32f7").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x8000 != 0) ratio = (ratio * 0x31be135f97d08fd981231505542fcfa6) >> 128;
    if !(abs_tick & U256::from(0x8000)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0x31be135f97d08fd981231505542fcfa6").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x10000 != 0) ratio = (ratio * 0x9aa508b5b7a84e1c677de54f3e99bc9) >> 128;
    if !(abs_tick & U256::from(0x10000)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0x9aa508b5b7a84e1c677de54f3e99bc9").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x20000 != 0) ratio = (ratio * 0x5d6af8dedb81196699c329225ee604) >> 128;
    if !(abs_tick & U256::from(0x20000)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0x5d6af8dedb81196699c329225ee604").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x40000 != 0) ratio = (ratio * 0x2216e584f5fa1ea926041bedfe98) >> 128;
    if !(abs_tick & U256::from(0x40000)).is_zero() {
        ratio = ratio.saturating_mul(U256::from_str("0x2216e584f5fa1ea926041bedfe98").unwrap())
            >> 128u32;
    }
    // if (absTick & 0x80000 != 0) ratio = (ratio * 0x48a170391f7dc42444e8fa2) >> 128;
    if !(abs_tick & U256::from(0x80000)).is_zero() {
        ratio =
            ratio.saturating_mul(U256::from_str("0x48a170391f7dc42444e8fa2").unwrap()) >> 128u32;
    }

    // if (tick > 0) ratio = type(uint256).max / ratio;
    if tick > 0 {
        ratio = U256::MAX.checked_div(ratio).unwrap();
    }
...
mod tests {
    ...
    use crate::{
        core::TickMath::{self, getTickAtSqrtRatio},
        getSqrtRatioAtTick,
    };
    use primitives::{Int24, U256};

    const MIN_TICK: Int24 = -887272;
    const MAX_TICK: Int24 = 887272;

    fn encodePriceSqrt(reserve1: U256, reserve0: U256) -> U256 {
        reserve1
            .integer_sqrt()
            .mul(U256::from(2).pow(U256::from(96)))
            .div(reserve0.integer_sqrt())
    }

    #[test]
    #[should_panic(expected = "T")]
    fn it_getSqrtRatioAtTick() {
        let _ = TickMath::getSqrtRatioAtTick(MIN_TICK - 1);
    }

    #[test]
    #[should_panic(expected = "T")]
    fn throws_for_too_low() {
        let _ = TickMath::getSqrtRatioAtTick(MAX_TICK + 1);
    }

    #[test]
    fn min_tick() {
        assert!(
            TickMath::getSqrtRatioAtTick(MIN_TICK) == U256::from_dec_str("4295128739").unwrap()
        );
    }
    ...
    fn min_tick_plus_1() {
        assert!(
            TickMath::getSqrtRatioAtTick(MIN_TICK + 1) == U256::from_dec_str("4295343490").unwrap()
        );
    }
    ...
    fn min_tick_ratio_is_less_than_js_implementation() {
        assert!(
            TickMath::getSqrtRatioAtTick(MIN_TICK)
                < encodePriceSqrt(U256::one(), U256::from(2).pow(U256::from(127)))
        );
    }

    #[test]
    fn max_tick_ratio_is_less_than_js_implementation() {
        assert!(
            TickMath::getSqrtRatioAtTick(MAX_TICK)
                > encodePriceSqrt(U256::from(2).pow(U256::from(127)), U256::one())
        );
    }
    ...
    //     500_000,
    //     738_203,];
    //     for absTick in ticks{
    //         for tick in -absTick..absTick{
    //             // const jsResult = new Decimal(1.0001).pow(tick).sqrt().mul(new Decimal(2).pow(96))
    //             let jsResult = U256::from_dec_str(&1.0001f64.powi(tick).sqrt().mul(2u128.pow(96) as f64).to_string()).unwrap();
    //             let result = TickMath::getSqrtRatioAtTick(tick);
    //             let absDiff=
    //             if result >= jsResult {
    //                 result.sub(jsResult)
    //             }else{
    //                 jsResult.sub(result)
    //             };
    //             assert!(absDiff.div(jsResult).mul(U256::from(1000000))<U256::zero());
    //         }
    //     }
    // }
    ...
    #[test]
    fn it_tick_some_value() {
        let tick_index = -276324;
        println!("tick_index is:{}", tick_index);
        let price = getSqrtRatioAtTick(tick_index);
        println!("price is:{}", price);
        let tick = getTickAtSqrtRatio(price);
    ...
    //this test case is error still, TODO test.
    #[test]
    fn just() {
        // 79228162514264337593543
        let ratio = U256::from_dec_str("79228162514264337593543").unwrap();
        let tick = getTickAtSqrtRatio(ratio);
        println!("tick is:{:?}", tick);
        let ratioOfTick = getSqrtRatioAtTick(tick);
        println!("ratioOfTick is:{:?}", ratioOfTick);
        assert!(ratio.ge(&ratioOfTick));
    }
    ...
    fn it_work() {
        ...
        i6 = U256::from_big_endian(&[0xff_u8; 32])
            .saturating_sub(i6)
